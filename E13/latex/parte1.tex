...
Per completezza ricordiamo che è possibile anche ottenere la $f(t)$ utilizzando le trasformate di Fourier. È infatti sufficiente calcolare la trasformata per un solo rettangolo e poi fare il prodotto di convoluzione con la $\delta$ tenendo conto della periodicità. 

$$f(t)=\sum_{n=-\infty}^{+\infty}\frac{1}{2}\frac{sin(n/2)}{n/2}\delta(\omega-\frac{n}{\tau})$$

Il risultato finale è lo stesso, sebbene ci sia un abuso di notazione in quanto la $\delta$ è una distribuzione e ha senso solo sotto il segno di integrale. Sarebbe più corretto utilizzare una delta di Kronecker.
...
Come vediamo anche in questo caso abbiamo solo le armoniche dispari, ovvero $\nu_n=\frac{2n+1}{\tau}=\frac{2n+1}\nu_0$. Osservando la FFT dei dati vediamo che ciò è anche in questo caso vero. Anche in questo caso non vale il teorema del campionamento.
