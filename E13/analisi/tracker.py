...
import numpy as np
	...
	####  Qui vanno i dati 	####
dataG1 = np.genfromtxt("../dati/scope_15.csv", delimiter=',')
...
# Titolo del grafico
fig1.suptitle("Risposta di un ADC di tipo tracking", y=0.97, fontsize=15)
...
f1.grid(True)
f1.set_ylim((-1.5, 0.2000))
f1.set_xlim((-0.75,.75))

f1.set_ylabel(u'd.d.p. [$V$]', labelpad=0, fontsize=14)
f1.set_xlabel(u'Tempo [$s$]', labelpad=0, fontsize=14)

f1.legend((g1, g2 ), (r'$V_{DAC,input}$', r'$V_{DAC,output}$'), 'lower right', prop={'size': 15})
    
######

# questo imposta i bordi del grafico
fig1.subplots_adjust(left=0.09, right=0.98, top=0.93, bottom=0.09, hspace=0.085, wspace=0.05)
# mostra grafico
plt.show()
