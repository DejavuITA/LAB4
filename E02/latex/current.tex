...
\subsection{Correzioni a posteriori}
...
Nel prima parte dell'esperienza abbiamo tentato di misurarla dapprima non considerando le correnti (circuito in Figura \ref{cir:inv}) e poi tentando di minimizzarle sfruttando una resistenza posta fra comune ed ingresso non invertente data dall'annullamento di (\ref{eq2:Vout_currents}) (circuito in Figura ?????????).
Supponiamo che la corrente di offset
$$I_{of} = |I_{b^+}|-|I_{b^-}| \approx 0$$
Vale dunque che l'approssimazione fatta per trovare la resistenza